...
from modules import ClassPrototypes, ClassPrototypes2
...
class PatchMerging(nn.Module):
    def __init__(self, input_resolution, dim, out_dim, activation):
        ...
        self.act = activation()
        self.conv1 = Conv2d_BN(dim, out_dim, 1, 1, 0) # Increase channels (dim -> out_dim)
        self.conv2 = Conv2d_BN(out_dim, out_dim, 3, 2, 1, groups=out_dim) # Decrease resolution (strde=2)
        self.conv3 = Conv2d_BN(out_dim, out_dim, 1, 1, 0)

    def forward(self, x):
        if x.ndim == 3: # Transformer format [B, N, C]
            H, W = self.input_resolution
            B = len(x)
            # Then convert the input in an image format (B, C, H, W)
        ...
        # Come back to Transformers format
...
class ConvLayer(nn.Module):
    def __init__(self, dim, input_resolution, depth,
        ...
        self.decrease_token = decrease_token
        #self.pool = nn.AdaptiveAvgPool1d(dim)
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            x = self.downsample(x)
        if self.decrease_token:
            x = self.proj(x.transpose(1, 2))  # [B, C, N_in] → [B, C, N_out]
            x = x.transpose(1, 2)  # [B, C, N_out] → [B, N_out, C]
        return x
...
class SAHARA(nn.Module):
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                layer = ConvLayer(
                    conv_expand_ratio=mbconv_expand_ratio,
                    decrease_token=(img_size_source>img_size_target),
                    **kwargs,
                )
            else:
                layer = BasicLayer(
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                layer = ConvLayer(
                    conv_expand_ratio=mbconv_expand_ratio,
                    decrease_token=(img_size_target>img_size_source),
                    **kwargs,
                )
            else:
                layer = BasicLayer(
        ...
        self.domain_cl = nn.Linear(embed_dims[-1] // 2, 2)

        # Task Classifier head
        self.norm_head_task = nn.LayerNorm(embed_dims[-1] // 2)
        self.task_cl = nn.Linear(embed_dims[-1] // 2, num_classes)
    ...
    def forward_source(self, x, source):
        ...
        return emb_inv_norm, emb_spec_norm, self.domain_cl(emb_spec_norm), self.task_cl(emb_inv_norm)
